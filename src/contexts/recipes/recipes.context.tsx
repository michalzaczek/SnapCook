...
function RecipesProvider({ children }: { children: ReactNode }) {
    ...
    const [recipes, setRecipes] = useState<IRecipe[]>(() => {
        ...
        if (storage) {
            ...
            const recipesForEqualQuery = localStorageRecipes
                .find(recipeStorage =>
                    selectedIngredients.every(i => recipeStorage.queryIngredients.includes(i)) &&
    ...
    async function fetchAndSet(ingredients: string[]): Promise<void> {
        ...
        if (storage) {
            ...
            const recipesForEqualQuery = localStorageRecipes
                .find(recipeStorage =>
                    ingredients.every(i => recipeStorage.queryIngredients.includes(i)) &&
        ...
        localStorage.setItem(localStorageKey, JSON.stringify([{ recipes, queryIngredients: ingredients }]));
        setRecipes(recipes);
